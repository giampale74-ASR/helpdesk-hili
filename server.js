...
passport.use(new GoogleStrategy({
  ...
  const user  = dbQueryOne('SELECT * FROM users WHERE email=? AND attivo=1', [email]);
  if (!user) {
...
passport.deserializeUser((id, done) => {
  const user = dbQueryOne('SELECT * FROM users WHERE id=?', [id]);
...
app.get('/auth/google/callback',
  ...
  (req, res) => {
    // Login riuscito: imposta la sessione manuale (stessa logica del login normale)
